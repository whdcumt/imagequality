...
for j=1:size(letter,2),
    ...
    result(j,1)=psnr(M{j},Y{j});
    result(j,2)=psnr(G{j},Y{j});
    result(j,3)=psnr(B{j},Y{j});
end
 xlswrite('G:\研二所有论文相关\研二上的一篇Retinex去雾论文\论文中用到的图片\论文中用到的质量评价标准程序\PSNR.xls',result,'sheet1');
